...
class OllamaProvider(BaseProvider):
        ...
        **kwargs
        ...
        try:
            ...
            if response.status_code != 200:
                return {error: fHTTP response.status_code}
    ...
    def health_check(self) -> Dict[str, Any]:
        try:
            response = requests.get(fself.url/api/tags, timeout=5)
            if response.status_code == 200:
                models = [m[name] for m in response.json().get(models, [])]
                return {healthy: True, models: models}
            return {healthy: False, error: fHTTP response.status_code}
        except Exception as e:
    ...
    def generate(
        ...
        **kwargs
        ...
        try:
            start = time.time()
            response = requests.post(
                fself.base_url/chat/completions,
                headers={
